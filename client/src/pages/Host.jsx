...
export default function Host({ onBack }) {
  ...
  const importQuestionsFromPdf = async (file) => {
    ...
    try {
      ...
      const response = await fetch(`${SERVER_URL}/api/questions/import-pdf`, {
        method: 'POST',
        body: formData
      })
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Could not import questions from PDF')
      }

      setQuestions(prev => [...prev, ...data.questions])
      setImportMsg(`Imported ${data.count} question${data.count === 1 ? '' : 's'} from PDF`)
    } catch (err) {
